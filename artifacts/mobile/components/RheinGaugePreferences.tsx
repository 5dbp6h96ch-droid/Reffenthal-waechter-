...
export default function RheinGaugePreferences({ userId }: { userId: string | null | undefined }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const { data, error } = await (supabase as any)
        ...
        .eq('user_id', userId);
      if (!cancelled) {
        setSelectedIds(error ? [] : (data ?? []).map((row: { gauge_id: string }) => row.gauge_id));
  ...
  const toggleGauge = async (gauge: Gauge) => {
    ...
    setMessage(null);
    const next = selectedIds.includes(gauge.id)
      ? selectedIds.filter((id) => id !== gauge.id)
      : [...selectedIds, gauge.id];
    setSelectedIds(next);
    setSaving(true);
    const { error: deleteError } = await (supabase as any)
      .from('user_preferred_gauges').delete().eq('user_id', userId);
    if (deleteError) {
      setSaving(false); setSelectedIds(selectedIds); setMessage('Auswahl konnte nicht gespeichert werden.'); return;
    }
    if (next.length > 0) {
      const { error: insertError } = await (supabase as any)
        .from('user_preferred_gauges')
        .insert(next.map((gaugeId) => ({ user_id: userId, gauge_id: gaugeId })));
      if (insertError) {
        setSaving(false); setSelectedIds(selectedIds); setMessage('Auswahl konnte nicht gespeichert werden.'); return;
      }
    }
    setSaving(false);
    setMessage(`${next.length} Pegel ausgewählt.`);
  };
    ...
    void (async () => {
      const entries = await Promise.all(selected.map(async (g) => {
        try {
          ...
          return [g.id, { available: false, value: null, timestamp: null } as ForecastInfo] as const;
        }
      }));
    ...
    return (
      <View style={{ backgroundColor: colors.card, borderRadius: 12, borderWidth: 1, borderColor: colors.border, padding: 16, gap: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <Text style={{ fontSize: 14, fontFamily: 'SpaceGrotesk_600SemiBold', color: colors.foreground }}>Anmeldung erforderlich</Text>
        </View>
  ...
  return (
    <View style={{ gap: 10 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Text style={{ fontSize: 11, fontFamily: 'SpaceGrotesk_600SemiBold', color: colors.mutedForeground, letterSpacing: 2, textTransform: 'uppercase' }}>Meine Rheinpegel</Text>
        <Text style={{ fontSize: 11, fontFamily: 'SpaceGrotesk_500Medium', color: colors.primary }}>{selectedIds.length} ausgewählt</Text>
      </View>
      <TextInput
        value={search}
        onChangeText={setSearch}
        placeholder="Rheinpegel suchen …"
        placeholderTextColor={colors.mutedForeground}
        style={{ fontSize: 14, fontFamily: 'SpaceGrotesk_400Regular', color: colors.foreground, backgroundColor: colors.muted, borderRadius: 9, paddingHorizontal: 12, paddingVertical: 10, borderWidth: 1, borderColor: colors.border }}
      />
            ...
            return (
              <TouchableOpacity key={g.id} disabled={saving} onPress={() => void toggleGauge(g)} activeOpacity={0.7} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: idx === filtered.length - 1 ? 0 : 1, borderBottomColor: colors.border, backgroundColor: selected ? colors.primary + '08' : 'transparent' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
                  <View style={{ width: 30, height: 30, borderRadius: 15, backgroundColor: selected ? colors.primary + '20' : colors.muted, alignItems: 'center', justifyContent: 'center' }}>
                    <Feather name={selected ? 'check' : 'anchor'} size={14} color={selected ? colors.primary : colors.mutedForeground} />
                  </View>
                  <View style={{ flex: 1, gap: 2 }}>
                    <Text style={{ fontSize: 14, fontFamily: selected ? 'SpaceGrotesk_700Bold' : 'SpaceGrotesk_500Medium', color: selected ? colors.primary : colors.foreground }}>{g.name}</Text>
                    <Text style={{ fontSize: 11, fontFamily: 'SpaceGrotesk_400Regular', color: colors.mutedForeground }}>{`Rhein · km ${(g.river_km ?? 0).toFixed(2).replace('.', ',')}`}</Text>
                    {selected && forecast && (
                      <Text style={{ fontSize: 10, fontFamily: 'SpaceGrotesk_500Medium', color: forecast.available ? colors.safe : colors.mutedForeground }}>
                        {forecast.available ? (forecast.value != null ? `Vorhersage: ${forecast.value} cm${forecast.timestamp ? ` · ${new Date(forecast.timestamp).toLocaleString('de-DE', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}` : ''}` : 'Vorhersage verfügbar') : 'Keine Vorhersage verfügbar'}
                      </Text>
                    )}
                  </View>
                </View>
